...
output "required_groups" {
  ...
  value       = var.groups.create_required_groups == false ? tomap({ for key, value in var.groups.required_groups : key => value.id }) : tomap({ for key, value in module.required_group : key => value.id })
}

output "optional_groups" {
  description = "List of Google Groups created that are optional to the Example Foundation steps."
  value       = var.groups.create_optional_groups == false ? tomap({ for key, value in var.groups.optional_groups : key => value.id }) : tomap({ for key, value in module.optional_group : key => value.id })
}
